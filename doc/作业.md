...
1. `current_led = 1U` 改成 `2U`，观察从哪颗 LED 开始；*（第二颗）*
2. `BEEP_MS` 从 `120U` 改成 `300U`，观察蜂鸣器响声长短；*变成长了*
3. `delay_ms -= 20U` 改成 `+= 20U`，看速度变化方向；*理论上速度会越来越大，实际上感知不明显*
4. 删掉或改掉 `beep(BEEP_MS)` 的调用，观察程序行为。 *删掉：不叫了*
